...
<div class="min-h-screen bg-[#2a126e20] dark:bg-[#aa91ed20]">
    {{-- Hero Section con Immagine --}}
    <section class="relative bg-gradient-to-b from-background to-primary text-text">
        ...
        <div class="relative max-w-[1000px] mx-auto px-4 py-32 lg:px-0">
            <div class="max-w-4xl">
                {{-- Categorie --}}
                @if ($project->categories->count() > 0)
                    <div class="flex flex-wrap gap-2 mb-4">
                        @foreach ($project->categories as $category)
                            <span
                                class="px-3 py-1 bg-secondary text-background-contrast dark:text-text rounded-full text-sm">
                                {{ $category->name }}
                            </span>
                        @endforeach
                    </div>
                ...
                <h1 class="text-3xl md:text-5xl font-bold mb-5">{{ $project->title }}</h1>
                <p class="text-lg text-text bg-background bg-opacity-80 border border-muted p-2.5  mb-2.5 rounded-lg">
                    {{ $project->description }}
                </p>
                ...
                <div class="flex flex-wrap gap-2.5">
                    @if ($project->project_url)
                        <a href="{{ $project->project_url }}" target="_blank" rel="noopener noreferrer"
                            class="inline-flex items-center px-6 py-3 bg-accent text-background-contrast dark:text-text font-semibold rounded-lg hover:brightness-90 transition">
                    ...
                    @if ($project->github_url)
                        <a href="{{ $project->github_url }}" target="_blank" rel="noopener noreferrer"
                            class="inline-flex items-center px-6 py-3 bg-gray-800 text-background-contrast dark:text-text font-semibold rounded-lg hover:brightness-90 transition">
    ...
    <section class="mx-auto px-4 py-5 lg:px-0 max-w-[1000px]">
        <div class="grid lg:grid-cols-3 gap-2.5">
            {{-- Contenuto --}}
            <div class="lg:col-span-2">
                {{-- Descrizione completa --}}
                @if ($project->content)
                    <div class="bg-background border border-background-contrast rounded-lg p-5 mb-2.5">
                ...
                @if ($allImages && count($allImages) > 1)
                    <div class="bg-background border border-background-contrast rounded-lg p-5">
                        <h2 class="text-2xl font-bold mb-2.5">Gallery</h2>
                            ...
                            @foreach ($allImages as $index => $image)
                                <div wire:click="openImageModal({{ $index }})"
                                    ...
                                    <div
                                        ...
                                        <svg class="w-12 h-12 text-text opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            ...
            <div class="lg:col-span-1">
                {{-- Dettagli Progetto --}}
                <div class="bg-background border border-background-contrast rounded-lg shadow-sm p-5 sticky top-[70px]">
                    <h3 class="text-lg font-bold mb-4">Details</h3>

                    @if ($project->client)
                        <div class="mb-4">
                            <p class="text-sm text-text brightness-75">Client</p>
                    ...
                    @if ($project->start_date || $project->end_date)
                        <div class="mb-4">
                            <p class="text-sm text-text brightness-75">Working Period</p>
                    ...
                    @if ($project->technologies->count() > 0)
                        <div class="mb-4">
                            <p class="text-sm text-text mb-2">Technologies</p>
                    ...
                    {{-- Share buttons --}}
                    <div class="pt-4 border-t border-muted border-opacity-40">
                        <p class="text-sm mb-3">Share</p>
                        <div class="flex gap-2">
                            <button wire:click="shareProject('facebook')"
                                class="p-2 bg-blue-600 text-white rounded-lg hover:brightness-90 transition">
                            ...
                            <button wire:click="shareProject('twitter')"
                                class="p-2 bg-sky-500 text-white rounded-lg hover:brightness-90 transition">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                            ...
                            <button wire:click="shareProject('linkedin')"
                                class="p-2 bg-blue-700 text-white rounded-lg hover:brightness-90 transition">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                            ...
                            <button wire:click="copyProjectLink"
                                class="p-2 bg-secondary text-background-contrast dark:text-text rounded-lg hover:brightness-90 transition">
    ...
    @if ($relatedProjects && $relatedProjects->count() > 0)
        <section class="bg-background border-t border-background-contrast py-20">
            <div class="mx-auto px-4 lg:px-0 max-w-[1000px]">
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-2.5">
                    @foreach ($relatedProjects as $related)
                        <a href="{{ route('portfolio.show', $related->slug) }}"
                            class="group bg-background-contrast rounded-lg overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                            ...
                            <div class="p-4">
                                <h3 class="font-bold text-lg mb-1 group-hover:text-accent transition">
                                    {{ $related->title }}
                                </h3>
