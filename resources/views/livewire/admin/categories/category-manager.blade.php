<div class="bg-[#2a126e20] dark:bg-[#aa91ed20]">
    <div class="max-w-[1000px] mx-auto h-full w-full py-2.5 px-4 lg:px-0 min-h-screen">
        ...
        <div class="mb-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <button wire:click="toggleForm"
                    class="px-4 py-2 bg-accent text-background-contrast dark:text-text rounded-lg hover:brightness-90 transition">
        ...
        @if ($showForm)
            <div class="bg-background rounded-lg p-6 mb-6" wire:key="form-section">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <input type="text" wire:model="color" placeholder="#3B82F6"
                                    class="w-32 border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="bg-background rounded-lg overflow-hidden">
                    ...
                    <div class="space-y-2">
                        @foreach ($categories as $category)
                            <div wire:sortable.item="{{ $category->id }}" wire:key="cat-drag-{{ $category->id }}"
                                ...
                                <div class="w-8 h-8 rounded-full" style="background-color: {{ $category->color }}">
                                </div>
                                <div class="flex-1">
            ...
            @else
                ...
                <table class="min-w-full divide-y divide-muted">
                        ...
                        <tr>
                            ...
                            <th class="px-3 py-3">
                                <button wire:click="sortBy('name')"
                                    class="text-xs font-bold text-text uppercase hover:text-gray-700">
                                    Nome
                                    @if ($sortField === 'name')
                            ...
                            <th class="px-3 py-3 text-left text-xs font-bold text-text uppercase">
                                Descrizione
                            </th>
                            ...
                            <th class="px-3 py-3 text-xs font-bold text-text uppercase">
                                Progetti
                            </th>
                            <th class="px-3 py-3 text-xs font-bold text-text uppercase">
                                Azioni
                    ...
                    <tbody
                        class="bg-background divide-y divide-muted transition-all ease-in-out duration-200 font-mono">
                        @forelse($categories as $category)
                            <tr class="hover:bg-background-contrast">
                                <td class="px-3 py-3">
                                ...
                                <td class="px-3 py-3">
                                    <span
                                        class="px-2 py-1 text-xs rounded-full {{ $category->projects_count > 0 ? 'bg-primary text-background-contrast dark:text-text' : 'bg-muted text-background-contrast dark:text-text' }}">
                                        {{ $category->projects_count }}
                                    </span>
                                </td>
                                <td class="py-3 px-3 h-full text-sm inline-flex gap-2.5 items-center justify-center">
                                    <button wire:click="clone({{ $category->id }})"
                                        class="text-text hover:text-accent px-3 py-1.5 border border-muted hover:border-accent rounded-md bg-background-contrast hover:brightness-95 font-sans"
                                        title="Clona">
                                        Clona
                                    </button>
                                    <button wire:click="edit({{ $category->id }})"
                                        class="text-text hover:text-blue-500 px-3 py-1.5 border border-muted hover:border-blue-500 rounded-md bg-background-contrast hover:brightness-95 font-sans">
                                        Modifica
                                    </button>
                                    <button wire:click="delete({{ $category->id }})"
                                        wire:confirm="Eliminare {{ $category->name }}?"
                                        class="text-text hover:text-red-500 px-3 py-1.5 border border-muted hover:border-red-500 rounded-md bg-background-contrast hover:brightness-95 font-sans">
                                        Elimina
                                    </button>
                                </td>
                            </tr>
        ...
        @if (session()->has('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
                class="fixed bottom-4 right-4 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded z-50">
                {{ session('success') }}
            </div>
        @endif
        ...
        @if (session()->has('error'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                class="fixed bottom-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded z-50">
                {{ session('error') }}
            </div>
        @endif
    </div>
